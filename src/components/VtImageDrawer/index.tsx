...
export default class VtImageDrawaer extends PolygonDrawer {
  ...
  render() {
    return (
      <div
        ...
        class={[
          'vtx-imageDrawer',
          this.drawingRect || this.drawingPolygon ? 'is-drawing' : ''
        ]}
        style={{
          width: this.width ? `${this.width / 100}rem` : '100%',
          height: this.height ? `${this.height / 100}rem` : '100%'
        }}
      >
        {this.imageUrl &&
          <img
        ...
        />
        {
          this.$slots.buttons && this.showButtons &&
          <div
            class="vtx-imageDrawer__buttons"
            style={{
              right: `${this.buttonsPosition.right}px`,
              top: `${this.buttonsPosition.top}px`
            }}
          >
  ...
  removeMovingLine() {
    ...
    this.clearCanvas();
    this.lines.forEach((line: { start: any; end: any; }) => {

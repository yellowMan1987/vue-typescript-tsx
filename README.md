...
// 在modules中生成一个模块文件夹 包含index.tsx和style.scss 
npm run new ModuleFile ModuleFile

// 在modules的子文件夹Module如 Search 内生成业务组件文件夹 包含index.tsx和style.scss 
npm run new ModuleFile ComponentName 

// 在modules的子文件夹Module如 Search 内生成业务组件的子组件生成文件夹 包含index.tsx和style.scss 
npm run new ModuleFile ComponentName ChildComponentName
```
...
export default class ComponentName extends Vue {
  ...
  render(h) {              // jsx语法中的render函数,渲染dom结构.
    return (
      <div></div>
    );
  }
